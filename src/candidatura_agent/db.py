...
class Database:
    ...
    def connect(self) -> Iterator[sqlite3.Connection]:
        """Abre, confirma/retrocede e fecha uma conexão SQLite por operação.

        `sqlite3.Connection.__exit__` não fecha a conexão; sem este wrapper o
        dashboard acumulava descritores a cada snapshot e acabava impedindo o
        ciclo horário de abrir o banco.
        """
    ...
    def asset_queue(self, limit: int = 10, stage: str | None = None) -> list[dict[str, Any]]:
        ...
        O filtro de etapa permite que resolução de URL e geração de CV avancem
        em paralelo; uma fila grande de URLs não pode bloquear CVs já prontos.
        """
        if stage not in (None, "resolve", "resume"):
            raise ValueError(f"etapa de ativo inválida: {stage}")
        with self.connect() as conn:
            rows = conn.execute(
                """SELECT j.*,
